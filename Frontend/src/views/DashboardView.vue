  ...
  <div class="dashboard">
    ...
    <div class="sidebar">
      ...
      <nav class="nav-menu">
        ...
        <div v-if="user && (user.role?.toLowerCase() === 'admin' || user.role?.toLowerCase() === 'manager')">
          <a href="#" class="nav-item">
            <div class="nav-icon">
              <svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
            </div>
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-info">
            ...
            <p>{{ user?.['solde_congés'] != null ? user['solde_congés'] + ' jours restants' : 'Non disponible' }}</p>
          </div>
        </div>
        <div class="stat-card">
          <span class="icon">💻</span>
...
export default {
  ...
  methods: {
    ...
    async handlePasswordChange() {
      ...
      try {
        const res = await fetch(`${API_URL}/api/utilisateurs/password`, {
          method: 'PUT',
          headers: {
...
.header-loading { font-size: 14px; color: #9ca3af; }
.header-content { display: flex; align-items: center; justify-content: space-between; }
.header-left h1 { font-size: 24px; font-weight: 700; color: #111827; margin: 0 0 5px 0; letter-spacing: -0.4px; }
.header-subtitle { font-size: 13px; color: #9ca3af; margin: 0; text-transform: capitalize; }
.header-badge {
  background: linear-gradient(135deg, #14b8a6 0%, #0d9488 100%);
  color: white; font-size: 12px; font-weight: 600; padding: 6px 16px;
  border-radius: 20px; letter-spacing: 0.4px; text-transform: capitalize;
...
.icon { font-size: 26px; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; background: #f0fdfa; border-radius: 12px; flex-shrink: 0; }
.stat-info label { font-size: 11px; color: #9ca3af; text-transform: uppercase; letter-spacing: 0.5px; font-weight: 600; display: block; }
.stat-info p { font-weight: 700; margin: 5px 0 0; color: #111827; font-size: 14px; }

/* ── Grille basse ───────────────────────────────────── */
.dashboard-core { display: grid; grid-template-columns: 2fr 1fr; gap: 18px; }

.actions-panel, .profile-security-card, .company-life {
  background: white; padding: 22px 24px; border-radius: 14px;
  box-shadow: 0 1px 6px rgba(0,0,0,0.05); border: 1px solid #f3f4f6;
}
.actions-panel h3, .profile-security-card h3, .company-life h3 {
  margin: 0 0 16px 0; font-size: 15px; font-weight: 700; color: #111827;
  padding-bottom: 12px; border-bottom: 1px solid #f3f4f6;
}
.empty-state { padding: 28px 0; text-align: center; color: #c4c9d4; font-size: 13px; }
.empty-state p { margin: 0; }
.side-panels { display: flex; flex-direction: column; gap: 18px; }

.secure-info { display: flex; flex-direction: column; }
.secure-info p { margin: 0; padding: 8px 0; font-size: 13px; color: #4b5563; border-bottom: 1px solid #f9fafb; display: flex; gap: 6px; }
.secure-info p:last-child { border-bottom: none; }
.secure-info strong { color: #6b7280; font-weight: 600; min-width: 60px; }
...
.btn-logout {
  width: 100%; padding: 9px 16px; background: #fff0f0; color: #ef4444;
  border: 1px solid #fecaca; border-radius: 8px; cursor: pointer;
  font-size: 13px; font-weight: 600; transition: background 0.2s ease, color 0.2s ease;
}
...
.modal-card {
  ...
  width: 420px; max-width: 90vw; box-shadow: 0 20px 60px rgba(0,0,0,0.18);
}
.modal-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 20px; }
.modal-header h3 { margin: 0; font-size: 16px; font-weight: 700; color: #111827; }
.modal-close { background: none; border: none; font-size: 16px; color: #9ca3af; cursor: pointer; padding: 4px 8px; border-radius: 6px; transition: background 0.15s; }
.modal-close:hover { background: #f3f4f6; color: #374151; }

.modal-error { background: #fef2f2; color: #dc2626; border-left: 4px solid #ef4444; padding: 10px 14px; border-radius: 8px; font-size: 13px; margin-bottom: 16px; }
.modal-success { background: #f0fdfa; color: #0d9488; border-left: 4px solid #14b8a6; padding: 10px 14px; border-radius: 8px; font-size: 13px; margin-bottom: 16px; }

.form-group { margin-bottom: 16px; }
.form-group label { display: block; font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px; }
.form-group input { width: 100%; padding: 10px 12px; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 14px; color: #111827; outline: none; transition: border-color 0.15s; box-sizing: border-box; }
.form-group input:focus { border-color: #14b8a6; box-shadow: 0 0 0 3px rgba(20,184,166,0.12); }
